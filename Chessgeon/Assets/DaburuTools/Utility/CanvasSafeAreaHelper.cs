...
namespace DaburuTools
{
	public class CanvasSafeAreaHelper : MonoBehaviour
	{
		...
		private static Vector2 wantedReferenceResolution = new Vector2(1080.0f, 1920.0f);
		...
		void Awake()
		{
			...
			Debug.Assert(_safeAreaTransform != null, "_safeAreaTransform is not assigned.");
		...
		public static Vector2 SafeAreaSize()
		{
			for (int i = 0; i < helpers.Count; i++)
			{
				if (helpers[i]._safeAreaTransform != null)
				{
					return helpers[i]._safeAreaTransform.sizeDelta;
				}
			}
